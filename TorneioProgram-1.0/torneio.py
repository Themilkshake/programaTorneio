...
def findOponent(jogadores, indexComp, total, byePresente, embates):
   ...
   empates2 = []
    ...
    ## se so sobraram duas pessoas, e uma delas eh bye, bye tem que ser o oponente
   if (len(jogadores[indexComp].lastAdvAdv) == embates-2) and byePresente:
       for i in range(0, total):
           if jogadores[i].name == "Bye":
               if i not in jogadores[indexComp].lastAdvAdv:
                   return i
...
def resolveNaoAleatorio(jogadores, totalPessoas, rodadas, byePresente):
   ...
   if(rodadas > 0):
       ...
       while(totalPessoas > 0): 
           print(str(placeholder-(totalPessoas-1)) + "o lugar:")
